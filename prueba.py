...
def reconocimiento():
    ...
    while True:
        ...
        for (x,y,w,h) in faces:
            sql = "SELECT codigo FROM personas WHERE x <= %s AND %s <= x + w AND y <= %s AND %s <= y + h"
            val = np.array((x, x, y, y)).astype(int)
            cursor.execute(sql, tuple(int(i) for i in val))
            result = cursor.fetchone()
            val = (int(x), int(x+w), int(y), int(y+h))

            if result is not None:
                codigo = result[0]
                fecha = datetime.datetime.now().date()
                hora = datetime.datetime.now().time()
                anotacion = input("Ingrese una anotación para esta lectura: ")

                # Discard any unread results
                while cursor.nextset():
                    pass

                sql = "INSERT INTO registros (codigo, fecha, hora, anotacion) VALUES (%s, %s, %s, %s)"
                val = np.array((codigo, fecha, hora, anotacion))
                val = list(val)
                cursor.execute(sql, list(val))
                db.commit()

                sql = "SELECT nombre FROM personas WHERE codigo = %s"
                val = np.array((codigo,))
                ...
                cv2.putText(frame, "Lectura inválida", (x, y-10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 0, 255), 2)
